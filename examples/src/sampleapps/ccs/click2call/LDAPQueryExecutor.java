...
public final class LDAPQueryExecutor {
     ...
     * 
     * @param providerUrl URL of LDAP server. It should be of the form
    ...
    public LDAPQueryExecutor(String providerUrl, String baseDN, String securityPrincipal, String securityCredentails) {
        ...
        if (securityPrincipal != null && !"".equals(securityPrincipal) && securityCredentails != null
                && !"".equals(securityCredentails)) {
    ...
    public ArrayList<String[]> dirLookup(String nameToLookUp) throws NamingException {
        ...
        // method.
        String filter = "(|(sn=" + nameToLookUp + ")(givenName=" + nameToLookUp + "))";
        ...
        while (queryResult.hasMore()) {
            ...
            StringTokenizer tokens = new StringTokenizer(attr.substring(1, attr.length() - 1), ",");

            // userdata will hold name, number and mail id of matches found.
            String[] userdata = new String[DirTableConstants.TABLE_COLUMN_COUNT];

            while (tokens.hasMoreTokens()) {
                String elem = tokens.nextToken();
                // find the first colon in the phone number
                int pos = elem.indexOf(':');

                String pre = elem.substring(0, pos);
                String post = elem.substring(pos + 1);

                if (pre.endsWith("telephoneNumber")) {
                    userdata[DirTableConstants.NUMBER_COLUMN_INDEX] = post;
                } else if (pre.endsWith("mail")) {
                    userdata[DirTableConstants.MAIL_COLUMN_INDEX] = post;
                } else if (pre.endsWith("givenName")) {
                    if (userdata[DirTableConstants.NAME_COLUMN_INDEX] == null) {
                        userdata[DirTableConstants.NAME_COLUMN_INDEX] = post;
                    } else {
                        userdata[DirTableConstants.NAME_COLUMN_INDEX] =
                                post + " " + userdata[DirTableConstants.NAME_COLUMN_INDEX];
                    }
                } else if (pre.endsWith("sn")) {
                    if (userdata[DirTableConstants.NAME_COLUMN_INDEX] == null) {
                        userdata[DirTableConstants.NAME_COLUMN_INDEX] = post;
                    } else {
                        userdata[DirTableConstants.NAME_COLUMN_INDEX] =
