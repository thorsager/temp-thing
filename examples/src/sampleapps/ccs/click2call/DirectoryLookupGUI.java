...
/**
 * This class creates and manages directory lookup GUI of Click2Call application.
 * 
 * @author Avaya Inc.
 */
public final class DirectoryLookupGUI extends JFrame {

    private static final long serialVersionUID = 14656333322392842L;
     ...
     */
    protected DirectoryLookupGUI(final ActionListener actionListener) {
        ...
        JLabel jl =
                new JLabel("Please enter first name OR last name" + " of the person to look for and click \"Lookup\"",
                        JLabel.CENTER);
     ...
     */
    public void displayData(final ArrayList<String[]> data) {
        for (int i = 0; i < data.size(); i++) {
            tableModel.insertRow(0, data.get(i));
        }
    }
    ...
    public String getNameToLookup() {
        if (jt.getText() == null || jt.getText().isEmpty()) {
        	displayMessage(
                    JOptionPane.ERROR_MESSAGE, "Please enter a valid user name for Lookup.");
            return null;
        }
    ...
    /**
     * Prompts the user to modify (if required) the number to dial and returns
     * that number.
     * 
     * @return number to dial or null if there was a problem
     */
    public String getNumberToDial() {
        ...
        // if no row is selected, selectedRowIndex would be -1
        if (selectedRowIndex == -1) {
            displayMessage(JOptionPane.ERROR_MESSAGE, "Please select a table row " + "before attempting a call.");
            return null;
        }
        ...
        try {
            ...
            try {
                selectedNumber = (String) table.getValueAt(selectedRowIndex, DirTableConstants.NUMBER_COLUMN_INDEX);
            } catch (Exception e) {
                // Exception here just denotes that no number is available in
                // the table
            }
            ...
            numberToDial =
                    (String) JOptionPane.showInputDialog("If you need to modify the number to \n"
                            + "be dialed, please do so now. (You may \n" + "leave the leading + and whitespaces.)",
                            selectedNumber);

            return numberToDial;
        } catch (HeadlessException hle) {
            System.out.println("An exception occured while obtaining number to dial.");
            hle.printStackTrace();
            return null;
        }
    }
