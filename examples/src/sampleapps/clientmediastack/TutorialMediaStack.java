...
/**
 * Purpose: This tutorial application demonstrates how to use the Avaya Client Media
 * stack library. It illustrates how a Media Sink like Speaker and a Media
 * Source like Microphone can get access to the Audio RTP streams. The tutorial
 * application registers a device (extension) with the Connector Server and
 * then waits for a call to come in. From a real phone, when a call is made to
 * this device, the device (application) answers the call.  Incoming Audio RTP
 * Stream then goes to the Speaker. Audio from the Microphone goes to the
 * outgoing Audio RTP Stream.
 * 
...
public class TutorialMediaStack {
    ...
    private String cmapiServerIp;
    private String cmapiUsername;
    private String cmapiPassword;
    private String cmapiServerPort;
    private String cmapiSecure;
    private String cmapiTrustStoreLocation;
    ...
    private static final String PROP_FILE_RTP_DIRECTION = "app.rtp.direction";
    private static final String PROP_FILE_SERVER_IP = "cmapi1.server_ip";
    ...
    // within this range for each audio stream. Each audio stream requires two ports (for RTP & RTCP).
    // Note that it is not dependent on the addition of MediaSinks and/or MediaSources to the audio stream.
    ...
    private String local_udpport_min = "40000";
    private String local_udpport_max = "47999";
    ...
    public static void main(String[] args) {
        TutorialMediaStack app = new TutorialMediaStack();
        try {
            app.bootstrap();
            app.start();
    ...
    public void bootstrap() throws CstaException, Exception {
        ...
        ClassLoader cl = TutorialMediaStack.class.getClassLoader();
        URL appURL = cl.getResource("tutorialClientMediaStack.properties");
        Properties appProp = new Properties();
        appProp.load(appURL.openStream());
        callServer = appProp.getProperty(PROP_FILE_CALL_SERVER);
        ...
        direction = appProp.getProperty(PROP_FILE_RTP_DIRECTION, "XMIT_RCV");
     ...
     *
     * @throws Exception - if an Exception is generated by one of the
     * calls to the services, it is thrown to the caller.
     */
    public void start() throws Exception {
        System.out.println("Startup using switch=" + callServer + " ext="
                + extension + " pw=" + password);
        ...
        if ("XMIT_ONLY".equalsIgnoreCase(direction) || "XMITONLY".equalsIgnoreCase(direction)) {
            rtpDirection = RtpDirection.XMIT_ONLY;
        } else if ("RCV_ONLY".equalsIgnoreCase(direction) || "RCVONLY".equalsIgnoreCase(direction)) {
        ...
        audio = mediaFactory.createAudio(extension, rtpDirection);
        ...
        // Create local Codec (default is G.711U) and add it to local MediaInfo.
        ...
        // Create local Encryption (default is "both") and add it to local MediaInfo.
        ...
        System.out.println("setupAudio" +
            ": address=" + InetAddress.getByName(localRtpAddress.getAddress()) +
        ...
        // Now create a Media Source from the TutorialMicrophone
        // and attach it to the mediaSession as a source
        if (rtpDirection == RtpDirection.XMIT_RCV || rtpDirection == RtpDirection.XMIT_ONLY) {
            microphone = new TutorialMicrophone();
            mediaSession.attachSource(microphone);
            microphone.setCodec(codec, packetSize);
            System.out.println("Added Audio source to media session");
        }

        // Now create a Media Sink from the TutorialSpeaker
        // and attach it to the mediaSession as a sink
        if (rtpDirection == RtpDirection.XMIT_RCV || rtpDirection == RtpDirection.RCV_ONLY) {
            speaker = new TutorialSpeaker();
            mediaSession.attachSink(speaker);
            speaker.setCodec(codec, packetSize);
        
            System.out.println("Added Audio sink to media session");
        }
        ...
        station.register(password, DependencyMode.MAIN, MediaMode.CLIENT, null, null, true,
    ...
    /*
     * This is the Listener for Media Events. These events indicate
     * when the far end endpoint started or stopped media. Use these
     * events to start or stop media on the Client Media Stack.
     */
    	...
    	public void mediaStarted(MediaStartEvent startEvent){
    		try{
    			...
    			InetAddress remoteAddress = InetAddress.getByName(
    								startEvent.getRtpAddress().getAddress());
    			int remoteRtpPort =
    			...
    			System.out.println(" starting Audio: remote=" + remoteAddress+":"+remoteRtpPort
    					           +", codec="+remoteCodec+", packetsize="+packetSize
    					           +", protocol="+remoteEncryption.getProtocol());

    			// Start Audio on the Client Media Stack
    			// with the Far End Audio Information
    			audio.start(new InetSocketAddress(
    					remoteAddress, remoteRtpPort), new InetSocketAddress(
    ...
    private class MyAsyncRegistrationCallback implements AsynchronousCallback
	{
	    ...
	    {
			System.out.println("Signaling encryption="+resp.getSignalingEncryption());
